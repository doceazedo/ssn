<script lang="ts">
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (!refreshToken)
				throw Error(
					'Houve um erro com seu token de acesso. Por favor, tente se registrar novamente'
				);
...
<Form bind:fields on:submit={handleSubmit} {isLoading} {error} {links} submitLabel="Registrar" />
